...
contract Deal {
  ...
  function sendOrder(string memory goods, uint quantity) public payable {
    ...
    require(msg.sender == buyerAddr,"Sender not authorized");

    /// Increment the order sequence
    orderseq++;

    /// Create the order register
    orders[orderseq] = Order(goods, quantity, orderseq, 0, 0, Shipment(address payable , 0, 0, address(this), 0, 0, false), true);
  ...
  function queryOrder(uint number) public view returns (address buyer, string memory goods, uint quantity, uint price,
                                                        uint safepay, uint delivery_price, uint delivey_safepay) {
    /// Validate the order number
    require(orders[number].init,"Order number not found");

    /// Return the order data
    return(buyerAddr, orders[number].goods, orders[number].quantity,
           orders[number].price, orders[number].safepay,
           orders[number].shipment.price, orders[number].shipment.safepay);
  }
